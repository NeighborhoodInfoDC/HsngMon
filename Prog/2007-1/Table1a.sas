...
proc format;
  ...
  value lblB
    4 = '2006 Q1 - 2006 Q2'
    3 = '2005 Q2 - 2006 Q2'
    2 = '2001 Q2 - 2006 Q2 (annualized)'
    1 = '1996 Q2 - 2006 Q2 (annualized)';
...
data avg_sales;
  ...
  ** Calculate sales per year **;
  ...
  select ( put( saledate, dtrngA. ) );
    when ( '2006 Q1', '2006 Q2' ) avg_sales = num_sales;   /** Not annualized **/
...
data sales_chg;
  ...
  **** NB: Quarter to quarter change is not annualized *****;
  price_chg = 100 * %annchg( price2006_q1, price2006_q2, 1 );
  output;
  
  saledate = 3;
  price_chg = 100 * %annchg( price2005_q2, price2006_q2, 1 );
  output;

  saledate = 2;
  price_chg = 100 * %annchg( price2001_q2, price2006_q2, 5 );
  output;

  saledate = 1;
  price_chg = 100 * %annchg( price1996_q2, price2006_q2, 10 );
  output;
...
%Output_Table1( start_row=18, var=saleprice_adj, data=avg_sales, fmt=lblA. )

%Output_Table1( start_row=24, var=price_chg, data=sales_chg, fmt=lblB. )

** Condominiums **;

%Table1_data( ui_proptype='11' )

%Output_Table1( start_row=35, var=avg_sales, data=avg_sales )

%Output_Table1( start_row=41, var=saleprice_adj, data=avg_sales, fmt=lblA. )

%Output_Table1( start_row=47, var=price_chg, data=sales_chg, fmt=lblB. )
