/**************************************************************************
 ...
 clusters.  Writes to Excel file
   d:\DCData\Libraries\HsngMon\Prog\2007-3\S8summary_2007_3.xls 
...
data HsngMon.&rpt_file;
  ...
  ** as recommended by Kira Brown;
  ...
  else if cur_expiration_date >= intnx( 'qtr', &cur_rpt_date, 0, 'beginning' ) then do;
  ...
  label 
    rpt_status = "Contract status as of report date (&cur_rpt_date_fmt)"
...
%macro Write_dde( sheet=, range=, data=, var=, fopt=notab );
  ...
  filename xlsFile "&rpt_path\_tmp_&rpt_xls..&sheet..&range..txt" lrecl=256;
  **************/
...
%Write_dde( sheet=Titles, range=r6c2:r6c2, data=RptDate, var=hud_file )
%Write_dde( sheet=Titles, range=r7c2:r7c2, data=RptDate, var=rpt_update )
...
%let prev_rpt_file = S8summary_2006_4;
