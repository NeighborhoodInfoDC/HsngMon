...
proc print data=Bpermits.Bpermits_2006 n='Permits = ';
  where put( issued_date, qtr. ) in ( &qtrs ) and permit_type = 'NEW BUILDING' and use in ( 'SFD' );
  id permit_no;
...
proc print data=Bpermits.Bpermits_2006 n='Permits = ';
  where put( issued_date, qtr. ) in ( &qtrs ) and permit_type = 'NEW BUILDING' and use in ( 'MFD' );
  id permit_no;
  var issued_date permit_type use units full_addr;
  sum units;
...
proc tabulate data=Bpermits.Bpermits_2007 format=comma8.0 noseps missing;
  where put( issued_date, qtr. ) in ( &qtrs ) and permit_type = 'NEW BUILDING' and use in ( 'SFD', 'MFD' );
  class ward use;
  var units;
  table 
    /** Rows **/
