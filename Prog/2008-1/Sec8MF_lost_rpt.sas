...
%put start_date=&start_date end_date=&end_date g_s8_pres_period_lbl_b=&g_s8_pres_period_lbl_b;
...
%let rpt_file = S8lost_&g_rpt_yr._&g_rpt_qtr;

%S8preservation_data()

proc contents data=S8losses_det;

proc means data=S8losses_det sum;
  var lost_contracts lost_units;
  title2 "Total Losses (&g_s8_pres_period_lbl_b)";
  
proc freq data=S8losses_det;
  tables cur_tracs_status;
run;
...
data detail_dat (keep=loss_date buff rownum);
  ...
  tot_assisted_units = sum( tot_assisted_units, lost_units );
  tot_contracts = tot_contracts + lost_contracts;
  ...
  %line_out( 
    "Num. assisted units: " || 
    trim( left( put( lost_units, comma8.0 ) ) ) ||
...
proc report data=detail_dat list nowd noheader;
  ...
  title1 height=11pt "Section 8 Multifamily Report: Lost Contracts and Units (&g_s8_pres_period_lbl_b)";
